...
import type { FamilyMember, FamilyMemberStatus } from '@/application/ports/IFamilyService';
...
export function FamilyCard() {
  ...
  const { user } = useAuth();
  const { members, invites, loading, submitting, submitError, addMember, respondToInvite, removeMember } = useFamily();
  ...
  const myMembers = members.filter((m: FamilyMember) => m.ownerUserId === user?.uid);
  const memberOf = members.filter((m: FamilyMember) => m.ownerUserId !== user?.uid);
  ...
  return (
    <div className="bg-white rounded-2xl border border-purple-50 shadow-sm p-4 space-y-2">
      <div className="flex items-center justify-between">
        ...
        >
          {adding ? t('cancel') || 'Cancel' : t('familyAddMember') || '+ Add member'}
        </button>
      </div>
          ...
          {invites.map((invite) => (
            <div key={invite.id} className="flex items-center justify-between gap-2 text-[12.5px]">
              <span className="text-gray-800">
                {t('familyInviteFrom', { name: invite.ownerName || invite.name }) ||
                  `${invite.ownerName || invite.name} wants to add you as "${invite.relationship}"`}
              </span>
      ...
      {adding && (
        <form onSubmit={handleAdd} className="space-y-2 rounded-xl border border-gray-200 p-3">
          <p className="text-[10.5px] text-gray-500">
            {t('familyAddHelp') ||
              "If they already have a patient account, we'll send them an invite to accept. Otherwise we'll create an account for them and email them to set a password."}
          </p>
          {submitError ? (
          ...
          {memberOf.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-[10.5px] font-semibold text-gray-500 uppercase tracking-wide">
                {t('familyMemberOf') || "Families you're part of"}
              </p>
              {memberOf.map((member) => (
                <div key={member.id} className="flex items-center justify-between gap-2 rounded-xl border border-gray-100 px-3 py-2">
                  <div className="min-w-0">
                    <p className="text-[12.5px] font-medium text-gray-800 truncate">
                      {member.ownerName || t('familySectionTitle')}
                    </p>
                    <p className="text-[10.5px] text-gray-500 capitalize">{member.relationship}</p>
                  </div>
                  <button
