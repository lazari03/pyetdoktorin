...
export default function PharmacyLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      />
      <MobileMenu open={mobileMenuOpen} items={pharmacyNav} activePath={pathname ?? ''} onNavigate={handleNavClick} />
      <DesktopTopBar
        items={pharmacyNav}
        activePath={pathname ?? ''}
...
function MobileTopBar({
  ...
  return (
    <div className="md:hidden fixed top-0 left-0 right-0 bg-white shadow-md flex items-center justify-between px-4 py-4 z-20">
...
function MobileMenu({
  ...
  if (!open) return null;
  return (
    <div className="md:hidden fixed top-16 inset-x-0 bg-white shadow-lg border-t border-gray-100 z-20">
        ...
        {items.map((item) => (
          <button
            ...
            className={`text-left px-4 py-3 text-sm ${
              activePath === item.href ? 'text-purple-600 font-semibold' : 'text-gray-800'
            }`}
          >
            {item.name}
          </button>
