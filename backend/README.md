...
Node.js + Express service that centralizes authentication, user management, appointments, prescriptions, clinics, and payments. Uses Firebase Admin SDK for data access and PayPal for payment processing.
...
- PayPal creds can point to sandbox or live; keep the base URL in sync.
...
- `POST /api/payments/create-order` & `/capture-order` – PayPal order flow.
